...
- **Do NOT remove the idle-gate override for `planner_self` (#153).** When the plan wants idle but live grid is exporting >`plannerSelfIdleOverrideW` (1 kW), the gate flips off and reactive PI absorbs the unused surplus. Without it, any forecast error on the PV or load twin leaks surplus straight to the grid for the rest of the slot. `TestPlannerSelfIdleGateOverridesOnLargeLiveSurplus` is the regression guard.
