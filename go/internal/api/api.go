...
// worldwide, requires_key, license, note, covers}]}. `covers` is advisory:
// for a bounded source it is a lat/lon box test, so a true near a corner
// still means "worth trying", not "guaranteed". False is reliable — that
// location is definitely not served.
//
// This exists because every price provider is European and nothing
// previously said so: a site outside those areas got an empty result and
// no explanation. See #726 and docs/data-coverage.md.
func (s *Server) handleDataSources(w http.ResponseWriter, r *http.Request) {
	...
	// An explicit ?lat=&lon= overrides the configured site so the Weather
	// tab can preview coverage for a pin the operator is still dragging
	// around, before they save it.
