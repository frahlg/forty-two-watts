...
// The phase-switching + fuse-clamp tests. Safety-critical: never allow a
// commanded EV current that, at the chosen phase count, would exceed
// the site fuse. Backward compat: a loadpoint without PhaseMode must
// continue to act as a statically-3Φ install (what the easee driver
// has always assumed).
...
func TestAutoPicksOnePhaseBelowSplit(t *testing.T) {
	// 2.3 kWh over 1 h → wantW ≈ 2300 W, below 3680 split → 1Φ.
	// Nearest 1Φ step: 2300.
...
func TestAutoPicksThreePhaseAboveSplit(t *testing.T) {
	cfg := ftwLoadpoint("auto")
	// 6 kWh / 1 h = 6000 W, above 3680 split → 3Φ. Nearest 3Φ step: 6210 or 5520.
	// 6000 is closer to 6210 (Δ=210) than 5520 (Δ=480).
...
func TestLockedThreePhaseFiltersSteps(t *testing.T) {
	// Locked 3Φ with a small wantW that, if auto, would pick 1Φ. Instead
	// we force 3Φ and must never emit a 1Φ step.
	cfg := ftwLoadpoint("3p")
...
func TestFuseClampThreePhaseHoldsBelowCeiling(t *testing.T) {
	// Huge wantW should be clamped to 16 A × 230 V × 3 = 11040 W ceiling,
	// then snapped to nearest 3Φ step ≤ ceiling → 11000 W.
	cfg := ftwLoadpoint("3p")
...
func TestFuseClampWithSmallerFuse(t *testing.T) {
	// 10 A fuse → 1Φ ceiling 2300, 3Φ ceiling 6900. Auto with want=12000
	// should go 3Φ and clamp to ≤6900, nearest step 6900.
	cfg := ftwLoadpoint("auto")
...
func TestDefaultPhaseModeIsThreePhaseBackwardCompat(t *testing.T) {
	...
	// fleet has `phases: 3` statically on the driver and we must not
	// suddenly start commanding 1Φ on them.
	cfg := ftwLoadpoint("")
	cmd := runTick(t, cfg, SiteFuse{MaxAmps: 16, Voltage: 230}, 2000, 60, time.Now())
	if cmd.phases != 3 {
...
func TestPhaseHoldPreventsFlap(t *testing.T) {
	...
	// Two different directives per tick, one wants 6 kW (3Φ), next wants 2 kW (1Φ).
	...
	wantWh = 2000
	// Advance now by 30s only — still within 60s hold window.
	...
	// Advance past hold window (61s after last change).
...
func TestZeroFuseMeansNoClamp(t *testing.T) {
	...
	// clamp to 0. wantW well under fuse → command normally.
	cfg := ftwLoadpoint("auto")
